...
class ResolveCommissionRateTests(unittest.TestCase):
    ...
    def test_override_nonzero_wins(self):
        """Override 0.0250 + plan rate 0.0150 -> resolver returns 0.0250."""
        sb = _supabase_with(
            merchant_row={
                "id": "m1",
            ...
            plan_row={"plan_id": "starter", "commission_rate": 0.0150},
        )
    ...
    def test_override_null_uses_plan_default(self):
        """Override NULL + plan rate 0.0150 -> resolver returns 0.0150
        from plan_limits, not from a hardcoded default.

        After migration 082 the post-resolver plan default is 0.0150
        (1.5% sales fee) on every seeded tier (superseding 054's 0.0100).
        This test pins that the resolver delivers whatever plan_limits
        says rather than hardcoding a value."""
        sb = _supabase_with(
            merchant_row={
                "id": "m1",
    ...
    def test_plan_default_post_082(self):
        """Phase 1 contract pin: the post-migration-082 plan default is
        Decimal('0.0150') on every seeded tier. If migration 082 is
        reverted (back to 054's 0.0100), this test fails first."""
        for tier in ("starter", "growth", "pro", "business", "enterprise"):
            sb = _supabase_with(
                ...
                plan_row={"plan_id": tier, "commission_rate": 0.0150},
            )
            rate = resolve_commission_rate(SELLER, supabase=sb)
            self.assertEqual(
                rate, Decimal("0.0150"),
                f"tier {tier!r} should resolve to 1.5% sales fee post-082",
            )
