...
- UI, backend responses, or docs that imply per-project tokens are
  on-chain / live / tradable on Solana. See the Simulated Token Labeling
  rules below.

### Simulated Token Labeling (Door A — DO NOT regress)

Per-project tokens on DUM Club today are simulated. Every launch writes a
`SIM_` placeholder mint address; token trading runs on a DB-only ledger in
`backend/api/routes/market.py`. To prevent misleading users, the repo has
three hard rules:

1. **Single source of truth.** Simulated state is detected via
   `backend/services/token_mode.py::is_simulated_token()` on the backend
   and `frontend/lib/tokenMode.ts::isSimulatedToken()` on the frontend.
   Do NOT introduce new `.startsWith("SIM_")` checks anywhere else — use
   the helpers. A future flip to on-chain is a one-line change in the
   helpers.

2. **Every response that returns `token_mint_address` or derived market
   data must include `is_simulated` and `token_mode` fields.** See
   `token.py`, `launch.py`, `market.py`, `project_tokens.py` for the
   existing pattern. Do not remove those fields.

3. **Every surface that renders a per-project token price, market cap,
   volume, buy/sell button, or mint address must visibly label the demo
   state when `is_simulated` is true.** The `SimulatedTokenBanner`
   component and the inline "(demo)" labels on metric cards are the
   canonical patterns. Do not hide, dismiss, or soften them.

DUM Points are explicitly OUT of scope for these rules. DUM Points are a
real in-app balance with a real Stripe on-ramp; they have their own
optional on-chain claim path and do not get the simulated-token label.
