...
export default function SquareCallbackPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/merchant/square/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`)
      .then(async (res) => {
        if (res.ok) {
          setStatus("success");
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-950">
      <div className="text-center">
        {status === "processing" && (
          <>
            <div className="mb-3 text-lg font-bold text-white">Connecting Square...</div>
            <p className="text-sm text-zinc-400">Please wait</p>
          </>
        )}
        {status === "success" && (
          <>
            <div className="mb-3 text-lg font-bold text-emerald-400">Square Connected!</div>
