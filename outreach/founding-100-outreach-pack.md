# Founding 100 Outreach Pack
## DUM Club — Seller Recruitment Kit

---

## Loom Walkthrough Script (~90 seconds)

**Open on:** Your Topgun Maintenance storefront on DUM Club.

> "Hey — I'm Julian. I built DUM Club because I was tired of platforms taking 8-10% of every sale. I also run Topgun Maintenance, an aircraft maintenance shop in Morristown, NJ — so I'm the first merchant on my own platform.

> Here's what it looks like. This is my storefront. Customers can browse my services, book directly, and pay through Stripe. I keep almost everything — just a low 1.5% sales fee per order, no hidden cuts.

> Every purchase earns the customer DUM Points — loyalty rewards they can use at any business on the network. That means my customers discover your business, and yours discover mine. It's a shared loyalty network that grows with every merchant.

> Right now I'm recruiting the founding 100 merchants. You join free. After the founding period, you lock in founding pricing for life. That's it. Flat fee plus a low 1.5% sales fee. Stripe pays you direct.

> If you're on Whatnot paying 8% plus processing, or running your own site with no built-in traffic — this is the alternative. Click the link below and claim your spot."

**Close on:** The /merchant signup page showing founding spots remaining.

---

## DM Templates

### Variant 1: Whatnot / Commonsold Seller

**Subject (if email):** You're paying Whatnot $1,150/mo on $10K in sales. Here's $49/mo flat instead.

> Hey [Name] — I've been watching your streams on [Whatnot/Commonsold] and your [product category] is solid.

> Quick question: how much are you paying in platform fees per month? If you're doing $10K/mo on Whatnot, that's roughly $1,150 gone — 8% platform + 2.9% processing on every sale.

> I built DUM Club as the flat-fee alternative. $39-$299/month plus a low 1.5% sales fee, Stripe pays you direct. Keep more of every sale.

> I'm recruiting the founding 100 merchants right now. Founding merchants join free and lock in founding pricing for life. I'm merchant #1 — I run an aircraft maintenance shop on the platform myself.

> Here's a 90-second walkthrough: [Loom link]

> Interested? Claim your spot here: [dumclub.com/merchant]

> — Julian

---

### Variant 2: TikTok Shop / eBay Live Seller

**Subject (if email):** Your TikTok sales, zero platform cut. Founding spots open.

> Hey [Name] — saw your [product type] content on TikTok and it's great. You clearly know how to sell live.

> I'm building DUM Club — a live selling marketplace like Whatnot but with one difference: flat monthly fee plus a low 1.5% sales fee. You keep more of every sale. Stripe pays you direct to your bank.

> We also have built-in loyalty (DUM Points) that brings customers back automatically — no stamps, no apps, no extra work. Customers earn points at your store and can spend them at any store on the network.

> Right now I'm filling the founding 100 slots. Free to join, founding pricing locked in for life after the founding period. I'm seller #1 — running my own business on it.

> 90-second demo: [Loom link]
> Grab your spot: [dumclub.com/merchant]

> — Julian

---

### Variant 3: Local Service Business

**Subject (if email):** Stop paying for Google Ads. Get discovered for free + keep every dollar.

> Hey [Name] — I found your [business type] on Google and your reviews are impressive.

> Quick pitch: I built DUM Club as a local business marketplace with a flat fee plus a low 1.5% sales fee. Instead of paying Google to show up in search or running expensive ad campaigns, your business gets listed with your real deals and Google reviews — for free during the founding period, then $39/mo flat.

> The platform also includes a built-in loyalty program (DUM Points). Every customer who buys from you earns points they can spend at any business on the network. It's like a shared loyalty card that works across every local business — replaces direct mail, punch cards, and Yotpo ($199+/mo) in one flat fee.

> I'm merchant #1 — I run Topgun Maintenance LLC, an aircraft maintenance shop in Morristown, NJ. The first 100 merchants get in free and lock in founding pricing for life.

> Quick walkthrough: [Loom link]
> Claim your founding spot: [dumclub.com/merchant]

> — Julian

---

## Follow-Up Sequence

### Day 0 — Initial DM/Email
Send one of the three variants above based on seller type.

### Day 2 — Soft bump (only if no reply)

> Hey [Name] — just bumping this in case it got buried. Founding spots are filling up ([X] of 100 claimed). No pressure — just didn't want you to miss it if you were interested.

> The short version: flat fee, low 1.5% sales fee, Stripe direct payouts, built-in loyalty. Here's the 90-second walkthrough if you haven't seen it: [Loom link]

### Day 5 — Value-add follow-up (only if no reply)

> Last one from me — I put together a fee calculator that shows exactly what you'd save vs. Whatnot/Commonsold: [dumclub.com/business?tab=calculator]

> On $10K/mo in sales, you'd save over $12,000/year switching to a flat $49/mo. Just thought you'd want to see the math.

> Either way, good luck with [specific thing you noticed about their business]. Your [product/service] is great.

### Day 10+ — No further follow-up
Do not send more than 3 messages. Respect the silence. Add them to a "revisit in 30 days" list if their content stays active.

---

## Outreach Targets by Channel

### Whatnot (priority — direct competitors)
- Search by category: trading cards, sneakers, vintage, collectibles, toys
- Sort by "recently live" to find active sellers
- Target sellers with 100-5,000 followers (big enough to sell, small enough to want alternatives)
- Look for sellers who complain about fees in their stream titles or socials

### TikTok Shop
- Search #TikTokShop, #SmallBusinessTikTok, #LiveSelling
- Target accounts with 1K-50K followers actively selling products
- Avoid pure content creators — focus on people moving product

### eBay Live
- New feature, sellers are experimenting — they're open to alternatives
- Search eBay Live categories, note seller usernames, find them on Instagram/TikTok for DM

### Local Service Businesses (NJ/NY metro first)
- Google Maps search: auto detailing, pet grooming, personal training, tutoring, home cleaning
- Target businesses with 4+ star ratings and 20+ reviews
- These businesses already pay for visibility — DUM Club replaces that spend

---

## Daily Outreach Cadence

**Target: 40 DMs/day**

| Time Block | Activity | Volume |
|---|---|---|
| 9-10 AM | Research + list building (find 40 sellers) | 40 prospects |
| 10-11 AM | Send DMs batch 1 (Whatnot/Commonsold) | 15 DMs |
| 2-3 PM | Send DMs batch 2 (TikTok/eBay) | 15 DMs |
| 4-5 PM | Send DMs batch 3 (local service) | 10 DMs |
| 5-5:30 PM | Follow-ups on Day 2 and Day 5 sequences | As needed |

**Weekly target: 200 contacts, 5-10% reply rate = 10-20 conversations = 3-5 signups**

At this rate: 100 founding slots filled in ~5-7 weeks.
